...
| _id | string | 自动 | 主键 |
| name | string | 是 | 姓名 |
| role | enum | 是 | 角色：`admin` / `sales` / `manager` |
| phone | string | 否 | 手机号 |
...
contracts:   [project_id, expiry_date, is_active]
...
登录方式：小程序启动时自动获取微信openid，首次使用时由admin绑定到staff记录。
